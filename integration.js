...
const SEARCH_ENGINE_ID = 'a8d5f05c126635b55';
...
function _createQuery(entity, searchFilters) {
  ...
  searchFilters.forEach((filter) => {
    if (filter.value === true) {
      query += ` site:${filter.filterValue} OR `;
    }
  });
  return query;
}
...
function doLookup(entities, options, cb) {
  ...
  entities.forEach((entity) => {
    tasks.push(function (done) {
      _searchEntity(entity, NO_SEARCH_FILTER, options, done);
    });
  });
    ...
    results.forEach((result) => {
      if (_isMiss(result.body)) {
        ...
        lookupResults.push({
          ...
          data: {
            summary: [`${result.body.searchInformation.totalResults} tweets`],
